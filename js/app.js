document.addEventListener('DOMContentLoaded', () => {
    ...
    const ui = {
        showToast: (m, t='info') => { const c=document.getElementById('toast-container'); if(!c)return; const e=document.createElement('div'); e.className=`toast ${t}`; e.innerHTML=`<span>${t==='success'?'✔️':'ℹ️'}</span> ${m}`; c.appendChild(e); setTimeout(()=>e.classList.add('show'),10); setTimeout(()=>{e.classList.remove('show');e.addEventListener('transitionend',()=>e.remove());},3000); },
        ...
        updateQualityAnalyzer: () => {
            ...
            if (!data.context) suggestions.push("Fournir un contexte évite les réponses génériques.");
    ...
    const handlers = {
        ...
        globalClickHandler(e) {
            const target = e.target; const navBtn = target.closest('.nav-item'); if (navBtn) return ui.switchTab(navBtn.dataset.tab);
        ...
        globalInputHandler(e) {
            const target = e.target; const wInput = target.closest('#creator-section input, #creator-section textarea, #creator-section select');
            if (wInput) { state.formData[wInput.id] = wInput.type==='checkbox' ? wInput.checked : wInput.value; ui.updateQualityAnalyzer(); }
