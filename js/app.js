document.addEventListener('DOMContentLoaded', () => {
    ...
    const storage = {
        get: (key) => JSON.parse(localStorage.getItem(key)) || [], set: (key, value) => localStorage.setItem(key, JSON.stringify(value)),
        getPrompts: () => storage.get('prompts'), savePrompts: (p) => storage.set('prompts', p),
        getFolders: () => storage.get('prompt_folders'), saveFolders: (f) => storage.set('prompt_folders', f),
    };
    ...
    const ui = {
        ...
        updateQualityAnalyzer: () => { const analyzerEl = document.getElementById('quality-analyzer'); if (!analyzerEl) return; let score = 0; const suggestions = []; const data = state.formData; if (data.persona_role) score += 2; else suggestions.push("Définir un rôle précis améliore la spécialisation."); if (data.objective) { score += 2; if (data.objective.length < 30) suggestions.push("Un objectif plus détaillé est souvent plus efficace."); } else { suggestions.push("L'Objectif est la partie la plus critique."); } if (document.getElementById('use_chain_of_thought')?.checked) score += 2; else suggestions.push("Cochez 'Réfléchis étape par étape' pour les tâches complexes."); if (data.context) score += 2; if (data.structure) score += 1; if (data.constraints) score += 1; const hasContent = Object.values(data).some(v => typeof v === 'string' && v.length > 0); analyzerEl.classList.toggle('hidden', !hasContent); if (!hasContent) return; const sC=document.getElementById('score-circle'), sL=document.getElementById('score-label'), sCo=document.getElementById('suggestions-container'), sLi=document.getElementById('suggestions-list'); if(sC)sC.textContent=`${score}/10`; const sClass=score<4?'score-poor':score<7?'score-average':'score-good'; if(sC)sC.className=`score-circle ${sClass}`; if(sL)sL.textContent=score<4?'Qualité: Basique':score<7?'Qualité: Efficace':'Qualité: Experte'; if(sCo)sCo.classList.toggle('hidden',suggestions.length===0); if(sLi){sLi.innerHTML=''; suggestions.forEach(s=>{const li=document.createElement('li');li.className='suggestion-item';li.textContent=s;sLi.appendChild(li);});} },
        openPromptModal(promptId) { const prompt = storage.getPrompts().find(p => p.id === promptId); if (!prompt) return; const modal = document.getElementById('prompt-modal'), title = document.getElementById('modal-title'), body = document.getElementById('modal-body'); if (modal && title && body) { title.textContent = prompt.title; body.textContent = prompt.content; modal.classList.remove('hidden'); } },
        closePromptModal() { const modal = document.getElementById('prompt-modal'); if (modal) modal.classList.add('hidden'); },
    };

    const handlers = {
        init() {
            // *** CORRECTION : On attache les écouteurs au document entier ***
            document.addEventListener('click', handlers.globalClickHandler);
            document.addEventListener('input', handlers.globalInputHandler);
        ...
        globalClickHandler(e) {
            ...
            // *** CORRECTION : Logique de fermeture prioritaire et robuste ***
            if (target.id === 'prompt-modal' || target.closest('#modal-close-btn')) { return ui.closePromptModal(); }
            ...
            else if (target.closest('#import-data-btn')) { document.getElementById('import-data-input').click(); }
            const cardAction = target.closest('.prompt-card [data-action]');
            if (cardAction) { const a=cardAction.dataset.action, id=cardAction.closest('.prompt-card').dataset.id; if (a==='delete'){if(confirm('Supprimer?')){logic.deletePrompt(id);ui.renderPromptLibrary();}}else if (a==='copy'){const p=storage.getPrompts().find(p=>p.id===id);if(p)navigator.clipboard.writeText(p.content).then(()=>ui.showToast('Copié!','success'));}else if (a==='edit'){ state.editingPromptId=id; const p=storage.getPrompts().find(p=>p.id===id); ui.switchTab('freeform'); document.getElementById('freeform-prompt-title').value=p.title; document.getElementById('freeform-prompt-content').value=p.content; document.getElementById('freeform-prompt-category').value=p.category; document.getElementById('save-freeform-btn').textContent='Mettre à jour';} return; }
            const card = target.closest('.prompt-card'); if (card) { ui.openPromptModal(card.dataset.id); }
        },
        globalInputHandler(e) {
            const target = e.target;
            if (target.closest('#creator-section')) { state.formData[target.id] = target.type==='checkbox'?target.checked:target.value; ui.updateQualityAnalyzer(); }
            if (target.closest('#library-section')) { ui.renderPromptLibrary(); }
            const moveSelect=target.closest('[data-action="move"]'); if(moveSelect) { const pId=moveSelect.closest('.prompt-card').dataset.id; logic.updatePromptFolder(pId,moveSelect.value); if(state.currentFolderId!=='all')ui.renderPromptLibrary(); ui.showToast('Déplacé.','info'); }
        },
        importHandler(e) { const file = e.target.files[0]; if(!file)return; const r=new FileReader(); r.onload=function(ev){ try{const d=JSON.parse(ev.target.result); if(d.prompts&&d.folders&&confirm('Remplacer données?')){storage.savePrompts(d.prompts);storage.saveFolders(d.folders);ui.showToast('Import réussi!','success');ui.updatePromptCount();ui.switchTab('library');}}catch(err){ui.showToast('Fichier invalide.','error');}}; r.readAsText(file); },
    };
